...
def filter(event)
	if(event.get('type') )
	 eventtype=event.get('type')
        ...
        if((eventtype =='allkeysmacro' and  event.get('gk_id')))
        ...
        elsif(eventtype == 'allkeysmacro' and  event.get('silverkey'))
            ...
            if(event.get('ext_rel_details'))                                                       
            listgkrelatedparty=populateorgrelatedparty( event.get('ext_rel_details'))
            if !(listgkrelatedparty == [])
                event.set('relatedParty',listgkrelatedparty)
            end
            end

            if(event.get('int_secsk_details'))                                                     
            listsecsk=populateSecsk(event.get('int_secsk_details'))
                if !(listsecsk == [])
                event.set('secondarySk',listsecsk)
                end
            end

            if(event.get('ext_secsk_details'))                                               
            listsecsk=populateSecsk(event.get('ext_secsk_details'))
                if !(listsecsk == [])
                event.set('secondarySk',listsecsk)
                end
            end
           ...
           if(event.get('int_rel_details'))                                                         
			listgkrelatedparty=populateorgrelatedparty( event.get('int_rel_details'))
			if !(listgkrelatedparty == [])
				event.set('relatedParty',listgkrelatedparty)
			end
			end
